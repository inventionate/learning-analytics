...
# Daten laden
loadFromLocalRepo(names(asearch("name:benutzer")))
...
average_visit_day <- average_visit_hour <- average_visit_duration <- average_visit_devices <- average_visit_city <- average_visit_events <-visit_event_action_notes <- visit_event_action_videos <- visit_video_event_actions_pause <- visit_video_event_actions_jump <- visit_video_event_actions_cuepoint <- visit_video_event_actions_play <- visit_video_event_actions_speed <- total_visit_duration <- total_visits <- total_actions <- total_events <- NULL

for( user in seq_along(benutzer)) {
  
  visit_days <- NULL
  for(i in 1:benutzer[[user]]$totalVisits) {
    
    visit_days <- c(visit_days, as.character(lubridate::wday(lubridate::as_date(benutzer[[user]]$lastVisits[[i]]$serverDate), label = TRUE, abbr = FALSE)))
    
  }
  average_visit_day <- c(average_visit_day, names(which.max(table(visit_days))))
  
  visit_hours <- NULL
  for(i in 1:benutzer[[user]]$totalVisits) {
    
    visit_hours <- c(visit_hours, as.numeric(benutzer[[user]]$lastVisits[[i]]$visitServerHour))
    
    
  }
  average_visit_hour <- c(average_visit_hour, round(mean(visit_hours), 0))
  
  visit_durations <- NULL
  for(i in 1:benutzer[[user]]$totalVisits) {
    
    visit_durations <- c(visit_durations, lubridate::duration(as.numeric(benutzer[[user]]$lastVisits[[i]]$visitDuration, "seconds")))
  ...
  for(i in 1:benutzer[[user]]$totalVisits) {
    ...
    visit_devices <- c(visit_devices, benutzer[[user]]$lastVisits[[i]]$deviceType)
    
  }
  average_visit_devices <- c(average_visit_devices, names(which.max(table(visit_devices))))
  
  
  visit_cities <- NULL
  for(i in 1:benutzer[[user]]$totalVisits) {
    
    if ( is.null(benutzer[[user]]$lastVisits[[i]]$city) ) city <- NA
    else city <- benutzer[[user]]$lastVisits[[i]]$city
  ...
  average_visit_city <- c(average_visit_city, city_max)
  
  
  visit_events <- NULL
  for(i in 1:benutzer[[user]]$totalVisits) {
    
    visit_events <- c(visit_events, benutzer[[user]]$lastVisits[[i]]$events)
  ...
  for(i in 1:benutzer[[user]]$totalVisits) {
    ...
    t <- t + length(benutzer[[user]]$lastVisits[[i]]$actionDetails)
    
    for(e in seq_along(benutzer[[user]]$lastVisits[[i]]$actionDetails) ) {
      
      visit_event_actions <- c(visit_event_actions, benutzer[[user]]$lastVisits[[i]]$actionDetails[[e]]$eventCategory)
      
    }
    
  }
  visit_event_actions <- fct_count(visit_event_actions) %>% spread(f, n)
  ...
  for(i in 1:benutzer[[user]]$totalVisits) {
    ...
    for(e in seq_along(benutzer[[user]]$lastVisits[[i]]$actionDetails) ) {
      ...
      visit_video_event_actions <- c(visit_video_event_actions, benutzer[[user]]$lastVisits[[i]]$actionDetails[[e]]$eventAction)
      
    }
    
  }
  visit_video_event_actions <- fct_count(visit_video_event_actions) %>% spread(f, n)
  ...
  total_visit_duration <- c(total_visit_duration, lubridate::duration(as.numeric(benutzer[[user]]$totalVisitDuration), "seconds"))
  
  total_visits <- c(total_visits, benutzer[[user]]$totalVisits)
  
  total_actions <- c(total_actions, benutzer[[user]]$totalActions)
  
  total_events <- c(total_events, benutzer[[user]]$totalEvents)
...
datensatz_synthesise_users <- data_frame(
  ...
  average_visit_day = average_visit_day,
  
  average_visit_hour = average_visit_hour,
  
  average_visit_duration = average_visit_duration,
  
  average_visit_city = average_visit_city,
  
  average_visit_devices = average_visit_devices,
...
rmFromLocalRepo(names(asearch("name:datensatz_synthesise_users")), removeData = TRUE, removeMiniature = TRUE)
asave(datensatz_synthesise_users, archiveSessionInfo = FALSE)
